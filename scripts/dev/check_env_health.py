#!/usr/bin/env python3
"""
File: check_env_health.py
Path: scripts/dev/check_env_health.py
Role: Validate local Python environment health and emit actionable diagnostics.
Used By:
 - maintainers running local preflight checks
Depends On:
 - argparse
 - dataclasses
 - datetime
 ...
 - tempfile
Notes:
 - Defaults to `.exo_env` and supports optional toolchain auto-repair.
"""
...
def _check_env_layout(env_path: Path) -> CheckResult:
    ...
    if has_pyvenv and has_conda_meta:
        return CheckResult(
            ...
            recommended_fix=(
                "Recreate `.exo_env` from a single environment model. "
                "Remove `.exo_env` then rebuild with `python3 -m venv .exo_env`."
            ),
        )

    if not env_path.exists():
        return CheckResult(
            check_id="env_path_exists",
            status=STATUS_FAIL,
            evidence=f"Environment path `{env_path}` does not exist.",
            recommended_fix=(
                "Create `.exo_env` and install dependencies with: "
                "`python3 -m venv .exo_env && .exo_env/bin/pip install -r requirements.txt`."
            ),
        )

    if not has_pyvenv:
        return CheckResult(
            check_id="venv_layout_detected",
            status=STATUS_WARN,
            evidence=f"`{pyvenv_cfg}` is missing.",
            recommended_fix=(
                "If this should be a standard virtualenv, recreate `.exo_env` with "
                "`python3 -m venv .exo_env`."
            ),
        )
    ...
    return CheckResult(
        check_id="env_layout",
        status=STATUS_PASS,
        evidence="Environment layout is consistent.",
...
def _check_python_subprocess_import(python_bin: Path) -> CheckResult:
    ...
    if code != 0:
        return CheckResult(
            ...
            recommended_fix=(
                "Interpreter is missing low-level subprocess support. "
                "Recreate `.exo_env` with a healthy Python distribution."
            ),
        )
    return CheckResult(
        check_id="python_subprocess_import",
...
def _check_pip_health(python_bin: Path) -> CheckResult:
    ...
    if version_code != 0:
        return CheckResult(
            ...
            recommended_fix=(
                "Reinstall pip inside `.exo_env` (`python -m ensurepip --upgrade`) "
    ...
    if check_code != 0:
        ...
        if any(marker in check_out for marker in fatal_markers):
            return CheckResult(
                ...
                recommended_fix=(
                    "pip is not healthy in `.exo_env`. Repair or rebuild with: "
...
def _check_packaging_toolchain(python_bin: Path) -> CheckResult:
    ...
    if code != 0:
        return CheckResult(
            ...
            recommended_fix=(
                "Repair packaging toolchain in `.exo_env` with: "
                "`python -m pip install --upgrade pip setuptools wheel`."
            ),
        )
    return CheckResult(
...
def _upgrade_packaging_toolchain(python_bin: Path) -> CheckResult:
    ...
    if code != 0:
        return CheckResult(
            ...
            recommended_fix=(
                "Rebuild `.exo_env` from scratch if automatic toolchain repair fails."
            ),
        )
    return CheckResult(
        check_id="packaging_toolchain_repair",
...
def _check_mypy_available(python_bin: Path) -> CheckResult:
    ...
    if code != 0:
        return CheckResult(
            ...
            recommended_fix=(
                "Install dev dependencies in `.exo_env` if your workflow requires mypy."
            ),
        )
    return CheckResult(
        check_id="mypy_available",
...
def _print_human(payload: dict[str, Any], fix_hints_only: bool) -> None:
    print(f"Environment health: {payload['status']}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate `.exo_env` health before local workflow commands."
    )
    parser.add_argument(
        "--env-path",
        default=".exo_env",
        help="Environment path to validate (default: .exo_env).",
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    results.append(_check_env_layout(env_path))
    ...
    if not python_bin.exists():
        results.append(
            CheckResult(
                ...
                recommended_fix=(
                    "Recreate `.exo_env` and confirm the interpreter exists at "
                    "`.exo_env/bin/python`."
                ),
            )
        )
        ...
        if args.auto_upgrade_toolchain and (
            ...
            results.append(
                CheckResult(
                    ...
                    status=STATUS_PASS,
                    evidence="Re-ran pip/toolchain checks after automatic repair attempt.",
                    recommended_fix="No action needed.",
                )
            )
    ...
    results.append(
        _check_writable_path(
            ...
            fix_hint=(
                "Fix ownership/permissions for `.exo_env` so your user can write to it "
                "(for example: `chown -R $USER:$USER .exo_env`)."
            ),
        )
    )
    results.append(
