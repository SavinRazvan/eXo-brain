...
| Ingress safety governance | Turn requests must receive pre-model allow/deny/escalate decisions with reason codes and audit continuity | `src/api/routers/turns.py`, `src/policies/ingress_gates.py`, `src/integration/host_adapter.py`, `src/core/orchestrator.py` | turn execution endpoints (SSE/WS) plus planned ingress gate profile APIs | `tests/modules/api/test_slice3_playground.py`, `tests/modules/policies/test_ingress_gate_chain.py`, `tests/modules/core/test_host_adapter_input_flow.py` | `src/observability/tool_audit.py` (`turn_ingress_decision`) + release gates (entitlement/performance expansion still planned) |
...
| Governance customization depth | Customers can use predefined and custom governance controls under explicit entitlement boundaries | `src/tenancy/policy_overlay.py`, `src/api/routers/tenants.py`, `src/access_control/policy_engine.py`, `src/policies/risk_gates.py` | tenant policy APIs + planned gate/profile APIs | `tests/modules/api/test_slice4_tenant_policy.py`, access control tests, planned custom gate tests | `architecture-goals/ENTITLEMENT_MATRIX.md`, planned entitlement decision audit anchors |
| Governance performance guarantees | Governance controls must preserve p95 turn latency and explicit fail-safe behavior | `src/api/routers/turns.py`, `src/observability/metrics.py`, `src/resilience/*` | runtime control/metrics surfaces + planned ingress budget controls | observability/reliability suites + planned gate latency tests | RC signoff evidence + planned governance latency budget reporting |
...
| Entitlement operability | Tier claims must map to explicit enforceable controls | `src/api/routers/*`, `src/policies/*`, `src/config/settings.py` | tier-sensitive surfaces documented in `architecture-goals/ENTITLEMENT_MATRIX.md` | existing API/policy/audit suites + planned entitlement integration tests | `architecture-goals/ENTITLEMENT_MATRIX.md`, RC signoff + planned entitlement evidence |
...
| Ingress governance plane with predefined + custom gates | Needed for pre-model safety, customer-governed protocols, and monetizable safety depth | Baseline implemented for turn-entry gate chain + audit continuity; predefined profile APIs and custom gate surfaces remain planned |
