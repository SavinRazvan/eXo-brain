...
| Ingress safety governance | Turn requests must receive pre-model allow/deny/escalate decisions with reason codes and audit continuity | `src/api/routers/turns.py`, `src/integration/host_adapter.py`, `src/core/orchestrator.py`, `src/policies/risk_gates.py` | turn execution endpoints plus planned ingress gate profile APIs | `tests/modules/api/test_slice1_transport.py`, `tests/modules/policies/test_policy_risk_gates.py`, planned ingress gate integration tests | `src/observability/tool_audit.py`, entitlement evidence + release gates (planned expansion) |
