...
| Backlog reconciliation v3 P0-1 UI readiness smoke pack | Implemented (baseline) | Added one-command local readiness target (`make ui-smoke`), deterministic API/UI + Tool/Agent/Playground smoke runner (`scripts/ui/local_ui_readiness_smoke.py`), and operator runbook (`docs/operations/local-ui-readiness-smoke.md`). |
| Backlog reconciliation v3 P0-2 release evidence runtime bridge | Implemented (baseline) | Added local runtime snapshot export (`.local/ui-smoke-runtime-snapshots.json`) and linked advisory `Runtime Snapshots` section into RC signoff markdown + normalized JSON (`runtime_snapshots`). |
| Backlog reconciliation v3 P1-1 Tool Manager guided onboarding | Implemented (baseline) | Added Tool Manager onboarding steps, diagnostics panel, and reason-code remediation mapping for import/upload/validate/version flow while keeping API contracts unchanged. |
