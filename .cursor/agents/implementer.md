...
- `.local/implementation-control-center.html` (UI rendering of trackers)
...
6. Update trackers:
   - mark tasks as `done`, `blocked`, or `deferred`
   - append entry in `updates-log.md` with impact and next step
7. Stop with explicit handoff notes.
...
- `python -m pytest -q` (or targeted suites when explicitly scoped)
