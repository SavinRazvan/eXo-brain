...
> **Adapter note (historical — notebook names superseded):** `OpenAIAgentsRuntimeAdapter` supports stub (`planned_tool_call`) and SDK paths when configured. **Current** suite: `tutorial_01`–`tutorial_08`, `check_01`–`check_04`, `edge_01`–`edge_02` — see `notebooks/README.md` and `docs/plans/notebook-standards.md` (builders: `build_tutorials.py`, `build_checks.py`). Ignore legacy `01_idea_validation` / `10_*` names in this archived plan.
