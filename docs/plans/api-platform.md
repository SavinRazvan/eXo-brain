...
> **Adapter note (current state):** `OpenAIAgentsRuntimeAdapter` (`src/runtime/openai_agents_runtime.py`) supports both the test stub path (planned tool intents) and the real SDK path when `OPENAI_API_KEY` is present and tool wiring is configured. Canonical notebook validation now lives in `notebooks/01_idea_validation.ipynb` and module checks in `notebooks/10_*` through `notebooks/13_*` (generated by `notebooks/build_validation_notebooks.py`).
