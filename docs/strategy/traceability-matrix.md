...
| Governed execution boundary | Customers keep provider/model connectivity while eXo-brain owns policy, deterministic execution, audit, and runtime control | `src/runtime/runtime_adapter.py`, `src/api/routers/providers.py`, `src/api/routers/turns.py`, `src/api/routers/runtime_control.py`, `src/api/routers/audit.py` | provider registration plus turn/runtime/audit control surfaces | `tests/modules/api/test_slice_provider_registration.py`, `tests/modules/core/test_orchestrator_turn.py`, `tests/modules/api/test_runtime_control_api.py`, `tests/modules/api/test_audit_api.py` | `docs/api/customer-api-integration-guide.md` + `scripts/architecture/check_governance_consistency.py` |
...
| Runtime observability | Runtime behavior must be observable and diagnosable through APIs today and standard telemetry sinks by supported profiles | `src/observability/logging.py`, `src/observability/metrics.py`, `src/observability/tracing.py`, `src/observability/timeline.py`, `src/observability/telemetry_export.py` (optional OTLP HTTP), `src/api/bootstrap.py` (calls `configure_opentelemetry_exporters`) | runtime stats endpoints in `src/api/routers/runtime_control.py`; optional `GET /metrics` when `EXO_ENABLE_PROMETHEUS_METRICS=1` (`src/api/routers/prometheus_metrics.py`) | `tests/modules/observability/test_observability.py`, `tests/modules/observability/test_release_guardrails.py`, `tests/modules/core/test_scheduler_observability.py`, `tests/modules/observability/test_telemetry_export.py`, `tests/modules/api/test_app_factory_branches.py` (Prometheus router) | RC signoff evidence + observability test suites |
...
| Enterprise telemetry export interoperability | **Partial:** OTel/Prometheus-compatible *baseline* export without replacing audit/API diagnostics; full productization (collector E2E, rich catalog, certification) remains roadmap | `src/observability/logging.py`, `src/observability/metrics.py`, `src/observability/tracing.py`, **`src/observability/telemetry_export.py`** (OTLP HTTP traces/metrics), **`src/api/routers/prometheus_metrics.py`** (optional `exo_build_info` text), `src/api/app.py` (router registration), `src/api/bootstrap.py` | runtime observability controls + optional `GET /metrics`; OTLP push via env (no dedicated REST for OTLP) | **`tests/modules/observability/test_telemetry_export.py`**, **`tests/modules/api/test_app_factory_branches.py`**, plus existing observability/release suites | RC signoff + **gap:** collector E2E and exporter health gates still tracked in `execution-board-12-gaps.md` E04 |
...
| Northbound OpenAI-compatible gateway | Optional OpenAI-shaped client surface without bypassing ingress, entitlements, or deterministic tool execution | `src/api/routers/openai_gateway.py`, `src/api/schemas/openai_gateway_schemas.py`, `src/api/routers/turns.py` (`iter_governed_turn_dicts_for_transport`), `src/api/app.py`, `src/api/dependencies.py` | `POST /v1/chat/completions` when `EXO_ENABLE_OPENAI_COMPAT_GATEWAY=1`; header `X-eXo-Session-Id` | `tests/modules/api/test_openai_compat_gateway.py` | `docs/plans/northbound-v1-gateway.md`, customer guide §4.0, `docs/architecture/ARCHITECTURE.md` §3 |
...
| OTel/Prometheus interoperability path | Enterprise operations often require standard telemetry sinks in addition to local JSONL/in-memory diagnostics | **Partial:** OTLP HTTP export + optional Prometheus `/metrics` baseline shipped (`telemetry_export.py`, `prometheus_metrics.py`, tests above). **Remaining:** collector E2E in CI, expanded metric catalog, release certification hardening — `execution-board-12-gaps.md` E04 |
