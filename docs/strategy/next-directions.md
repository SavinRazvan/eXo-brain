...
| **Adapter packages on PyPI (core extraction)** | **Shipped (2026-05):** four lockstep wheels **0.1.1**; eXo-brain PyPI-only (no in-tree mirror). **Next:** publish automation hardening, Lane A universal package. | [exo_adapters_pypi_handoff.md](../handoffs/exo_adapters_pypi_handoff.md), [adapter-compatibility-matrix.md](adapter-compatibility-matrix.md), TRACEABILITY §3 |
