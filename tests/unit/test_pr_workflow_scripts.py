...
Path: tests/unit/test_pr_workflow_scripts.py
...
def test_review_script_writes_actor_attribution(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(sys, "argv", ["review.py", "--pr", "123", "--actor", "SavinRazvan"])
    assert module.main() == 0

    content = (tmp_path / ".local" / "review.md").read_text(encoding="utf-8")
    assert "Action-By: @SavinRazvan" in content
    assert "Reviewed-By: @SavinRazvan" in content
...
def test_prepare_script_writes_actor_attribution(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(sys, "argv", ["prepare.py", "--pr", "123", "--actor", "SavinRazvan"])
    assert module.main() == 0

    content = (tmp_path / ".local" / "prep.md").read_text(encoding="utf-8")
    assert "Action-By: @SavinRazvan" in content
    assert "Prepared-By: @SavinRazvan" in content
    assert "GitHub-Profile: https://github.com/SavinRazvan" in content
...
def test_merge_script_writes_actor_attribution(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(sys, "argv", ["merge.py", "--pr", "123", "--actor", "SavinRazvan"])
    assert module.main() == 0
